...
func pop_up_screen() -> void:
	visible = true
	var _tween: Tween = create_tween().parallel()\
		.set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_IN_OUT)
	_tween.tween_property(content_layer, "offset", Vector2.ZERO, .9)
	_tween.tween_property(color_rect.material, "shader_parameter/blur", 3.0, 1.0)
	_tween.tween_property(color_rect.material, "shader_parameter/brightness", .4, 1.0)
	_tween.chain().tween_callback(restart_button.grab_focus)

func pop_off_screen() -> void:
	var _tween: Tween = create_tween().parallel()\
		.set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_IN_OUT)
	_tween.tween_property(content_layer, "offset", Vector2(0, -540), .9)
	_tween.tween_property(color_rect.material, "shader_parameter/blur", 4.0, 1.0)
	_tween.tween_property(color_rect.material, "shader_parameter/brightness", .1, 1.0)
